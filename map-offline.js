...
(function(){
  ...
  function _addToLayerControl(map,rendered){
    ...
    try{
      ...
      // Catat layer file supaya tombol Reset bisa menghapusnya dari peta
      map._bwkFileLayers=map._bwkFileLayers||[];
      map._bwkFileLayers.push(rendered);
    }catch(e){}
  }

  // ====== Tombol: Reset semua file peta yang di-upload ======
  function buildResetButton(map,options){
    options=options||{};
    var btn=L.control({position:options.position||'topright'});
    btn.onAdd=function(){
      var div=L.DomUtil.create('div','bwk-reset-ctrl');
      div.innerHTML='<button id="bwkResetBtn" class="bwk-ctrl-btn bwk-ctrl-reset" title="Hapus SEMUA file peta (GPX/KML/GeoJSON) yang sudah di-upload">'+
        '<span class="bwk-ctrl-ic">🗑️</span><span class="bwk-ctrl-tx">Reset file</span></button>';
      L.DomEvent.disableClickPropagation(div);
      L.DomEvent.on(div,'click',function(e){
        L.DomEvent.stop(e);
        resetFilesFlow(map);
  ...
  function resetFilesFlow(map){
    if(!window.bwkMapFiles){
      _toast('Module map-files belum dimuat','err');
      return;
    }
    var files=window.bwkMapFiles.list()||[];
    if(files.length===0){
      _toast('Tidak ada file peta yang tersimpan','ok');
      return;
    }
    var names=files.map(function(f){return '• '+(f.name||f.filename||'file');}).join('\n');
    if(!confirm('Hapus SEMUA '+files.length+' file peta yang sudah di-upload?\n\n'+names+'\n\nTile peta offline hasil "Simpan area" TIDAK ikut terhapus. Tindakan ini tidak bisa dibatalkan.')){
      return;
    }
    // Lepas semua layer file dari peta & dari kontrol layer
    var rendered=(map&&map._bwkFileLayers)||[];
    rendered.forEach(function(r){
      var l=r&&r.layer?r.layer:r;
      try{if(map._bwkLayersCtrl&&map._bwkLayersCtrl.removeLayer)map._bwkLayersCtrl.removeLayer(l);}catch(e){}
      try{map.removeLayer(l);}catch(e){}
    });
    if(map)map._bwkFileLayers=[];
    window.bwkMapFiles.clear();
    _toast(files.length+' file peta dihapus dari perangkat','ok');
  }
  ...
  window.bwkMapOffline={
    ...
    addResetControl:buildResetButton,
    addToLayerControl:_addToLayerControl,
    resetFiles:resetFilesFlow,
    injectCSS:injectCSS,
