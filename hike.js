...
var _syncing=false,_lastCheckin=0,_resetting=false;
...
window.hikeCheckin=function(){if(_lastCheckin>0&&Date.now()-_lastCheckin<3000){toast('Tunggu beberapa detik sebelum check-in lagi','warn');return;}_lastCheckin=Date.now();var s=state(),r=route();if(!s)return;var row={pos:r[s.i].n,alt:r[s.i].el,at:new Date().toISOString()};s.last=Date.now();if(s.i<r.length-1)s.i++;put(KEY,s);var q=get(QKEY,[]);q.push(row);put(QKEY,q);try{localStorage.setItem('bwkTrailCheckins',String((+localStorage.getItem('bwkTrailCheckins')||0)+1))}catch(e){};render();if(!_syncing)sync();toast('Check-in tersimpan '+(navigator.onLine?'dan disinkronkan':'offline'),'ok')}
window.hikeReset=function(){if(_resetting)return;_resetting=true;render();var confirmed=confirm('Mulai ulang pendakian?\n\n• Progress saat ini akan direset\n• Data check-in sebelumnya tetap tersimpan di admin');_resetting=false;if(!confirmed)return;put(KEY,{i:0,started:Date.now(),last:Date.now()});render();toast('Pendakian dimulai ulang','ok')}
function sync(){
  if(_syncing)return;
  _syncing=true;
