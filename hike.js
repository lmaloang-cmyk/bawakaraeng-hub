...
function render(){
  var el=document.getElementById('hikeTracker');
  if(!el)return;
  var r=route(),s=state();
  if(!s){
    el.innerHTML='<div class="hike-card"><div><b>🥾 Mode Pendakian Offline</b><small>Simpan pos jalur, nomor SOS, checklist, dan panduan tersesat di perangkat.</small></div><button onclick="hikeStart()">Mulai Pendakian</button></div>';
    return;
  }
  var i=Math.min(s.i||0,r.length-1);
  var cur=r[i];
  var next=r[i+1];
  var done=Math.round(i/(r.length-1)*100);
  var ago=mins(s.last||s.started,Date.now());
  var eta=next?Math.max(15,Math.round((next.el-cur.el)/6+35)):0;
  var isPeak=i>=r.length-1;
  var checkinBtn='';
  if(isPeak){
    checkinBtn='<button onclick="hikeReset()" id="hikeResetBtn">🔄 Mulai Ulang Pendakian</button>';
  } else {
    var disabledAttr=_lastCheckin>0&&Date.now()-_lastCheckin<3000?' disabled style="opacity:0.5"':'';
    checkinBtn='<button onclick="hikeCheckin()"'+disabledAttr+'>✓ Check-in '+escapeHtml(cur.n)+'</button>';
  }
  el.innerHTML='<div class="hike-card active"><div class="hike-head"><div><span>🥾 Pendakian Aktif · Offline siap</span><b>'+escapeHtml(cur.n)+'</b><small>'+cur.el+' mdpl · check-in '+ago+' menit lalu</small></div><strong>'+done+'%</strong></div><div class="hike-bar"><i style="width:'+done+'%"></i></div><div class="hike-next">'+(isPeak?'🎉 Anda sudah mencapai puncak! Silakan reset untuk memulai pendakian baru.:(next?'Berikutnya: <b>'+escapeHtml(next.n)+'</b> · sekitar '+eta+' menit · '+next.el+' mdpl':''))+'</div><div class="hike-actions">'+checkinBtn+'<button class="alt" onclick="hikeOfflineGuide()">📴 Panduan Offline</button></div><small class="hike-note">Antrean: '+get(QKEY,[]).length+' check-in akan dikirim saat internet kembali.</small></div>';
}
window.hikeStart=function(){
  var r=route();
  if(!r.length){toast('Data jalur belum siap','err');return;}
  put(KEY,{i:0,started:Date.now(),last:Date.now()});
  render();
  toast('Mode pendakian dimulai · data tersimpan offline','ok');
}
window.hikeCheckin=function(){
  if(_lastCheckin>0&&Date.now()-_lastCheckin<3000){
    toast('Tunggu beberapa detik sebelum check-in lagi','warn');
    return;
  }
  _lastCheckin=Date.now();
  var s=state(),r=route();
  if(!s)return;
  var row={pos:r[s.i].n,alt:r[s.i].el,at:new Date().toISOString()};
  s.last=Date.now();
  if(s.i<r.length-1)s.i++;
  put(KEY,s);
  var q=get(QKEY,[]);
  q.push(row);
  put(QKEY,q);
  try{localStorage.setItem('bwkTrailCheckins',String((+localStorage.getItem('bwkTrailCheckins')||0)+1))}catch(e){};
  render();
  if(!_syncing)sync();
  toast('Check-in tersimpan '+(navigator.onLine?'dan disinkronkan':'offline'),'ok');
}
window.hikeReset=function(){
  if(!confirm('Mulai ulang pendakian?\n\n• Progress saat ini akan direset\n• Data check-in sebelumnya tetap tersimpan di admin'))return;
  put(KEY,{i:0,started:Date.now(),last:Date.now()});
  render();
  toast('Pendakian dimulai ulang','ok');
}
function sync(){
  if(_syncing)return;
  _syncing=true;
