...
(function(){
  ...
  function token(){
    try{
      ...
      return c.auth.getSession().then(function(r){
        ...
        if(t)return t;
        // Sesi Google kosong/kedaluwarsa ΓåÆ WAJIB coba anonim
        ...
        // Timeout atau error koneksi ΓåÆ fallback ke anonim, jangan throw
  ...
  function _bootOutbox(){
    try{
      ...
      // selalu fresh ΓÇö terutama bila sesi Google baru saja berhasil di-recovery.
  ...
  window.addEventListener('bwk:sos-outbox',function(ev){
    ...
    try{
      ...
      if(d.type==='dead'){
        _outboxPill('SOS gagal terkirim ΓÇö pakai jalur cadangan',true);
        return;
      }
      var n=d.pending;
  ...
  function ensureCheckin(){var host=document.getElementById('peta');if(!host||document.getElementById('trailCheckin'))return;var box=document.createElement('div');box.id='trailCheckin';box.style.cssText='margin-top:20px;padding:12px;background:var(--card,#fff);border:1px solid var(--line,#e6e9ef);border-radius:12px';box.innerHTML='<div style="font-size:13px;font-weight:700;margin-bottom:10px">\u270D\ufe0f Check-in Pos</div><div style="display:flex;gap:8px;margin-bottom:8px"><select id="ciPos" style="flex:1;padding:8px;border:1px solid var(--line,#e6e9ef);border-radius:6px;font-size:12px;background:var(--card,#fff);color:var(--ink,#20263a)"></select><button onclick="window.opsCheckin()" style="padding:8px 12px;background:#2b6fff;color:#fff;border:0;border-radius:6px;font-weight:700;font-size:12px;cursor:pointer">\u2705 Check-in</button></div><div id="ciStatus" style="font-size:11px;color:var(--sub,#69758a)"></div>';// FIX v17.3: panel Check-in Pos dimasukkan KE DALAM section #peta (tepat di bawah
// peta + tracker), bukan lagi sebagai saudara SETELAH section. Versi lama membuat
// panel mengambang di luar sistem view: tampil di semua halaman termasuk Beranda,
// dobel dengan isi Peta Jalur, dan tertutup nav bawah karena tidak dapat padding
// 120px milik .view.
var anchor=document.getElementById('hikeTracker');if(anchor&&anchor.parentNode===host){host.insertBefore(box,anchor.nextSibling);}else{host.appendChild(box);}var sel=document.getElementById('ciPos');POS.forEach(function(p){var opt=document.createElement('option');opt.value=p[0];opt.textContent=p[1];sel.appendChild(opt);});checkinStatus();}
  ...
  window.opsDashboard=function(){var b=document.getElementById('adminBody');if(!b)return;b.innerHTML='<div class="aempty">Memuat dashboard operasi\u2026</div>';api('/api/operations?action=admin',[]).then(function(d){
    ...
    b.innerHTML='<style>.ops-card{display:flex;gap:8px;align-items:center;justify-content:space-between;border:1px solid var(--line,#e4e8ef);border-radius:13px;padding:12px;margin:9px 0;background:var(--card,#fff)}.ops-card.danger{border-color:#e0154a;background:#fff5f7;color:var(--ink,#991b1b)}html.dark .ops-card{background:var(--card,#1a1f2e);color:var(--ink,#e5e9f0)}html.dark .ops-card.danger{background:#2d1517;border-color:#7f1d1d;color:#fca5a5}.aempty{text-align:center;padding:20px;color:var(--sub,#69758a);font-size:14px}</style><h2>\uD83C\uDD98 SOS Aktif ('+active.length+')</h2>'+cards+'<h3>\uD83D\uDCC4 Riwayat SOS</h3><ul style="max-height:150px;overflow-y:auto">'+hist+'</ul><h3>\u2705 Check-in Terakhir</h3><ul style="max-height:150px;overflow-y:auto">'+check+'</ul>';
  ...
  function addPermitVerify(){if(!isAdminClient())return;var p=document.getElementById('adminPanel');if(!p||document.getElementById('opsPermitVerifier'))return;var el=document.createElement('div');el.id='opsPermitVerifier';el.style.cssText='margin-top:20px;padding:12px;background:var(--card,#fff);border:1px solid var(--line,#e6e9ef);border-radius:12px';el.innerHTML='<div style="font-size:13px;font-weight:700;margin-bottom:10px">\uD83D\uDD10 Verifikasi SIMAKSI (QR)</div><div style="display:flex;gap:8px"><input type="text" id="qrCode" placeholder="Scan atau paste kode QR..." style="flex:1;padding:8px;border:1px solid var(--line,#e6e9ef);border-radius:6px;font-size:12px;background:var(--card,#fff);color:var(--ink,#20263a)" /><button onclick="window.opsVerifyPermit()" style="padding:8px 12px;background:#2b6fff;color:#fff;border:0;border-radius:6px;font-weight:700;font-size:12px;cursor:pointer">Cek</button></div><div id="qrStatus" style="margin-top:8px;font-size:11px;color:var(--sub,#69758a)"></div>';p.appendChild(el);}
