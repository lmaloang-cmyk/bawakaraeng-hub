/* Web Push SOS: peringatan darurat tetap masuk walau aplikasi tertutup / layar HP mati.
   ...
   Kunci di bawah HANYA dipakai sebagai last-resort setelah server gagal 2x.
   Sumber kebenaran utama: /api/push-subscribe GET (lihat _serverKey()). */
(function(){
...
var row={endpoint:j.endpoint,p256dh:j.keys.p256dh,auth:j.keys.auth,device:_dev(),name:_name(),active:true,updated_at:new Date().toISOString(),role:(_u&&_u.role)||'',user_email:(_u&&_u.email)||''};
   ...
   // FALLBACK: digunakan HANYA bila server /api/push-subscribe gagal 2x (cold-start/offline).
   // Isi ENV VAPID_PUBLIC di Vercel agar baris ini TIDAK PERLU dipakai.
