...
export async function verifySupabaseUser(req) {
  ...
  // Fallback to the URL already baked into index.html so token verification
  // keeps working even when SUPABASE_URL is missing from env.
  const url = String(process.env.SUPABASE_URL || 'https://ncoueeeskzslldppsbvx.supabase.co').trim();
